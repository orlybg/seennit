...
# https://github.com/rutherford/seennit
...
def download(s):
	...
	if hasattr(s, 'url'):
		...
		if match is not None:
			# grab lone image
			rand_name = [random.choice(string.lowercase) for x in range(7)]
...
if __name__ =='__main__':
	...
	r = praw.Reddit('Seennit v1 - downloads saved image links from Reddit user account')

	r.login(user, passwd)

	if save_dir is None:
		save_dir = raw_input('Enter base save dir: ')
	...
	while more: # while there is potentially more pages to fetch...
		...
		for s in saves: # cycle through page...
			...
			count+=1
			if total==1: # if record is first in page, record & set to new last record after page finished
				first = s.fullname
				write_to_me(r'(^last =)', first)
	...
	print 'total saves processed: %d' % total
